#include "../../include/op/CannyOp.h"
#include<iostream>
#include<math.h>
using namespace lzw;

CannyOp::CannyOp(PixelMtrix *pixelMtrix) : Op(pixelMtrix, OpMode::Canny_Op){

}

int CannyOp::F(int p,int q,int w){
    if(p<0){
        p=0;
    }
...
PixelMtrix *CannyOp::doOp(){
    ...
    for(int row=0;row<this->getPixelMtrix()->getHeight(); row++){
        for(int col=0; col<this->getPixelMtrix()->getWidth();col++){
            if(this->getPixelMtrix()->getPixel()[row][col].getColor()->getR()!=0){
                std::cout<<"1"<<" ";
            }
            else{
                std::cout<<this->getPixelMtrix()->getPixel()[row][col].getColor()->getR()<<" ";
            }
            ...
            if(this->getPixelMtrix()->getPixel()[29][0].getColor()->getR()!=0){
                std::cout<<"1"<<" ";
            }
            else{
                std::cout<<this->getPixelMtrix()->getPixel()[29][0].getColor()->getR()<<" ";
            }
